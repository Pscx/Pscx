...
.INPUTS
You can pipe any object to Invoke-Sudo. It will be serialized and available in the userScript as $Input.

.OUTPUTS
Whatever the scriptblock returns. Use explicit "return" in your scriptblock. 

.EXAMPLE
PS> Get-Process notepad | Invoke-Sudo { Stop-Process }

PS> Invoke-Sudo { return Get-Content 'C:\My Secret Folder\My Secret.txt'}

PS> $a=1; $b = Invoke-Sudo { $using:a+10 }; Write-Host "Sum returned: $b";
Sum returned: 11
...
function Invoke-Sudo {
	...
	if($NoElevate) { 
		...
		if ($LoadProfile -or ((sudo.exe --loglevel None config Powershellloadprofile).Split(" = ")[1] -like "*true*" -and -not $NoProfile)) {
			$sNoProfile = ""
		} else {
		...
		$result = $remoteCmd | & sudo.exe $arguments *>&1
...
Export-ModuleMember -function Invoke-Sudo, sudo -Variable gsudoVerbose
