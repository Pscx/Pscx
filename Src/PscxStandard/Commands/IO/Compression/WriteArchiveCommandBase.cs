...
*/
		...
		internal void OnWriteProgress(object sender, WriteEventArgs e)
		{
			var progress = new ProgressRecord(0, 
                String.Format(Pscx.Resources.Strings.ArchiveCompressing, e.InputFile),
				String.Format(Pscx.Resources.Strings.ArchiveProgress, e.BytesRead, e.TotalBytes))
                    {
                        CurrentOperation =
                            String.Format(Pscx.Resources.Strings.ArchiveDestination, e.OutputFile),
