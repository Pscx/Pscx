...
namespace Pscx.Commands
{
    ...
    {
        private List<Tuple<PropertyInfo, PscxPathAttribute>> _boundPaths;
        ...
        {
            _boundPaths = new List<Tuple<PropertyInfo, PscxPathAttribute>>();
            ...
            {
                PropertyInfo parameter = boundPath.Item1;

                string parameterName = parameter.Name;
                
                // retrieve [PscxPath] attribute
                PscxPathAttribute pathAttrib = boundPath.Item2;
        ...
        protected void CheckProviderConstraints(PscxPathInfo pscxPath, PscxPathAttribute pathAttrib, string parameterName)
        {
            ...
            var violations = new Collection<Tuple<PscxPathInfo, Type>>();
            ...
            foreach (Type typeConstraint in constraints)
            {
                if (!typeConstraint.IsAssignableFrom(providerType))
                {
                    ...
                    else
                    {
                        ...
                        violations.Add(new Tuple<PscxPathInfo, Type>(pscxPath, typeConstraint));
                    }
                }
                else
                {
            ...
            {
                foreach (Tuple<PscxPathInfo, Type> violation in violations)
                {
                    // write out verbose reason for failure.
                    WriteVerbose(String.Format("The constraint check for the interface or base class '{0}' against the provider '{1}' failed.", violation.Item2,
                                               pscxPath.Provider.Name));
                }
        ...
        private class Visitor : CmdletReflectionVisitor
        {
            ...
            public override void VisitMemberAttribute(object attribute)
            {
                ...
                {
                    var _boundPath = new Tuple<PropertyInfo, PscxPathAttribute>(CurrentProperty,
                        (PscxPathAttribute) attribute);
                    //_boundPath.Item1 = CurrentProperty;
                    //_boundPath.Item2 = attribute as PscxPathAttribute;
